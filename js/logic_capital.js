const Logic_capital = {
    ...
    // ✅ CAPITAL Password for validation
    CAPITAL_PASSWORD: "3838383838383838",  // "88888888" in hex
    
    // ✅ Session authentication flag
    isAuthenticated: false,

    init: function() {
        this.currentMeter = CAPITAL_PROFILE; 
    ...
    validateAARQ: function(hexStr) {
        // Check if CAPITAL password exists in AARQ
        // CAPITAL Password: "88888888" = 3838383838383838
        if (hexStr.toUpperCase().includes(this.CAPITAL_PASSWORD.toUpperCase())) {
            return true;
        }
        return false;
    },
    ...
    processPacket: function(hexStr) {
        ...
        // CAPITAL SPECIFIC: Check for NEXT FRAME request first
        if (hexStr.includes("E6E600C00281000000")) {
            ...
            if (this.pendingBillFrame2) {
                ...
                let payload = Utils.extractPayloadFroCAPITALemplate(frame2);
                if (payload) {
        ...
        for (let key in profile.static_replies) {
            if (hexStr.includes(key)) {
                ...
                let payload = Utils.extractPayloadFroCAPITALemplate(staticFrame);
    ...
    generateDynamic: function(tmpl, requestHex) {
        ...
        let payload = Utils.extractPayloadFroCAPITALemplate(fullFrame);
        if (payload) {
