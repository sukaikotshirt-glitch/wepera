...
function connectUsb() {
    log('🔌 Connecting FT232R USB...', 'info');
...
function disconnectUsb() {
    ...
    updateUsbStatus('disconnected');
    updateServerButton();
    log('🔌 FT232R USB Disconnected', 'warning');
}
...
function updateUsbStatus(status) {
    ...
    switch(status) {
        case 'connected':
            ...
            text.textContent = '✅ FT232R Connected';
            sub.textContent = 'FTDI Serial • 9600 baud';
        ...
        case 'connecting':
            text.textContent = '🔌 Connecting FT232R...';
        ...
        case 'disconnected':
        default:
            text.textContent = '❌ FT232R Not Connected';
            sub.textContent = 'Connect FT232R USB cable to begin';
            signal.textContent = '';
            btn.textContent = '🔌 CONNECT FT232R';
            break;
    }
}
...
function attemptReconnect() {
    if (reconnectAttempts >= MAX_RECONNECT_ATTEMPTS) {
        ...
        showToast('FT232R Connection Lost - Please reconnect manually', 'error');
        isReconnecting = false;
        reconnectAttempts = 0;
        updateUsbStatus('disconnected');
        updateServerButton();
...
function toggleServer() {
    if (!isUsbConnected) {
        log('⚠️ Connect FT232R first!', 'warning');
        showToast('Connect FT232R first!', 'warning');
        return;
    }
...
function startServer() {
    ...
    log('📡 Waiting for meter reading request via FT232R...', 'info');
}
...
function updateServerStatus() {
    ...
    if (isServerRunning) {
        ...
        text.textContent = '🟢 Server Running - Ready for Reading via FT232R';
    } else {
...
function onConnected(message) {
    ...
    updateServerButton();
    log('✅ FT232R Connected Successfully', 'success');
...
function onUsbAttached() {
    log('🔌 FT232R USB Cable Detected', 'info');
    showToast('FT232R Detected - Connecting...', 'success');
...
function onUsbDetached() {
    log('⚠️ FT232R USB Cable Disconnected!', 'error');
    showToast('FT232R Disconnected!', 'error');
    ...
    if (wasRunning) {
        log('⏹️ Server stopped due to FT232R disconnect', 'warning');
    }
}

// ✅ USB Error
function onUsbError(errorMessage) {
    log(`❌ FT232R Error: ${errorMessage}`, 'error');
    ...
    if (isUsbConnected && !isReconnecting) {
        ...
        updateUsbStatus('disconnected');
        showToast('FT232R Connection Failed - ' + errorMessage, 'error');
    }
}

// ✅ USB Permission Denied
function onUsbPermissionDenied() {
    log('❌ USB Permission Denied - Please allow FT232R access', 'error');
    showToast('Please allow FT232R USB access!', 'error');
    updateUsbStatus('disconnected');
}
...
window.onload = function() {
    ...
    log('🚀 Web Server Ready', 'success');
    log('📱 Connect FT232R USB cable to begin', 'info');
};